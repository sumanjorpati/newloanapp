...
export type IProps = {
  cashIns: any | undefined | null;
  onPagination: (key: number) => void;
  onSort: (current: any) => void;
  onOrder: (current: string) => void;
};

const CashInList = ({ cashIns, onPagination, onSort, onOrder }: IProps) => {
  const { t } = useTranslation();
  const { data, paginatorInfo } = cashIns! ?? {};
  const rowExpandable = (record: any) => record.children?.length;
  console.log('cashInlist data',data)
  ...
  const columns = [
    ...
    {
      title: t("table:table-item-cash-in-amount"),
      dataIndex: "amount",
      key: "amount",
      align: "center",
      ellipsis: true,
    ...
    },
    {
      ...
      render: (id: string) => (
        <ActionButtons
          id={id}
          editUrl={`${EMPLOYEE_ROUTES.CASH_IN}/${id}/edit`}
          deleteModalView="DELETE_CASH_IN"
        />
      ),
    },
  ];
