...
import { clockOutValidationSchema } from "./clock-out-validation-schema";
import ValidationError from "@components/ui/form-validation-error";
import { useTranslation } from "next-i18next";
import SelectInput from "@components/ui/select-input";
import { yupResolver } from "@hookform/resolvers/yup";
import { useCreateClockOutMutation } from "@data/clock-out/use-clock-out-create.mutation";
import { useUpdateClockOutMutation } from "@data/clock-out/use-clock-out-update.mutation";
...
export default function CreateOrUpdateClockOutForm({ initialValues }: IProps) {
  ...
  const { mutate: createClockOut, isLoading: creating } = useCreateClockOutMutation();
  const { mutate: updateClockOut, isLoading: updating } = useUpdateClockOutMutation();
    ...
    />
      ...
      <div className="mb-4 text-end">
        ...
        <Button loading={creating || updating}>
          {initialValues
            ? t("form:button-label-update-clockOut")
            : t("form:button-label-add-clockOut")}
        </Button>
      </div>
    </form>
  );
