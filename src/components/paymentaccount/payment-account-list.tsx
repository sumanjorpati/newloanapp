...
export type IProps = {
  paymentAccounts: any | undefined | null;
  onPagination: (key: number) => void;
  onSort: (current: any) => void;
  onOrder: (current: string) => void;
};

const PaymentAccountList = ({ paymentAccounts, onPagination, onSort, onOrder }: IProps) => {
  const { t } = useTranslation();
  const { data, paginatorInfo } = paymentAccounts! ?? {};
  const rowExpandable = (record: any) => record.children?.length;
  console.log('payment-accountlist data',data)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (id: string) => (
        <ActionButtons
          id={id}
          editUrl={`${TENANT_ROUTES.PAYMENT_ACCOUNT}/${id}/edit`}
          deleteModalView="DELETE_PAYMENT_ACCOUNT"
        />
      ),
    },
  ];
