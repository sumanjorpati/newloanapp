...
export type IProps = {
  loanPaids: any | undefined | null;
  onPagination: (key: number) => void;
  onSort: (current: any) => void;
  onOrder: (current: string) => void;
};

const LoanPaidList = ({ loanPaids, onPagination, onSort, onOrder }: IProps) => {
  const { t } = useTranslation();
  const { data, paginatorInfo } = loanPaids! ?? {};
  const rowExpandable = (record: any) => record.children?.length;
  console.log('loanPaidlist data',data)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (id: string) => (
        <ActionButtons
          id={id}
          editUrl={`${EMPLOYEE_ROUTES.LOAN_PAID}/${id}/edit`}
          deleteModalView="DELETE_LOAN_PAID"
        />
      ),
    },
  ];
